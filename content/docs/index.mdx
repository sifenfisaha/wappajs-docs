...
- **Testability.** `@wappajs/core/testing` ships `MockTransport` and `ScriptedProvider`,
  so you can unit-test a whole bot offline, with no WhatsApp and no LLM key.
...
- Analytics / admin UI
- Redis session store (recipe: a database session store, three methods over one JSON
  document per chat)
