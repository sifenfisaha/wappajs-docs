---
title: Twilio
description: Run a bot through Twilio's WhatsApp Business API — sandbox quickstart, webhook signature validation, and the 24-hour template window.
---

`TwilioTransport` (`@wappa/twilio`) runs your bot through **Twilio's WhatsApp Business
...
[`@wappa/cloud-api`](/docs/transports/cloud-api) to talk to Meta directly. Twilio
WhatsApp is **DM-only** — every mapped message has `isGroup: false` and
...
`@wappa/twilio` stacks up against Baileys and the Cloud API.
